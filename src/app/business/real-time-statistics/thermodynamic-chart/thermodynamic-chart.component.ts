...
export class ThermodynamicChartComponent implements OnInit, OnDestroy {
  ...
  public currentRegion: RegionEntity; // 当前区域
  ...
  private mapZoomEventListener: any; // 地图缩放事件
  private idSubscription: Subscription; // 更新停车场信息

  public isShowRoadState = false; // 默认不显示路况
  public isShowHeatMap = true; // 默认显示热力图
  public isShowParking = false; // 默认不显示停车场

  private heatMapDataList: Array<ParkingDynamicsInfoHeatMapDataEntity> = []; // 热力图列表
  private parkingList: Array<ParkingDynamicsInfoEntity> = []; // 停车场列表
  private markerList: Array<any> = []; // 气泡列表
  ...
  @ViewChild(CitySelectComponent) public citySelectComponent: CitySelectComponent;
  @ViewChild(ParkingSelectComponent) public parkingSelectComponent: ParkingSelectComponent;
  ...
  public onCitySelected(region: RegionEntity) {
    if (region && region.region_id === this.currentRegion.region_id) {
      return;
    }
    this.currentRegion = region;
    // 清空数据,如果已经存在先从地图中移除
  ...
  public onSelectParkingChanged(parkingInfo: ParkingDynamicsInfoEntity) {
    ...
    if (isNullOrUndefined(parkingInfo)) {
      // 为空说明清空搜索数据了
      // 如果已经存在且为停用状态，则先从地图中移除，否则缩小气泡关闭弹窗
      this.selectSearchContentInfoWindow && this.selectSearchContentInfoWindow.close();
      this.selectSearchContentInfoWindow = null;
      let selectMarkerInfo = null;
      if (this.selectMarker) {
        selectMarkerInfo = this.selectMarker.getExtData();
        isRemove = !this.markerList.some(markerItem => markerItem.getExtData().parking.parking_id === selectMarkerInfo.parking.parking_id);
  ...
  public onParkingLayerBtnClick() {
    ...
    if (this.isShowParking) {
      ...
      // 显示->隐藏
      if (matchMarker) {
        this.zIndex = matchMarker.getzIndex();
        this.selectSearchContentInfoWindow && this.selectSearchContentInfoWindow.close();
      }
  ...
  private onMarkerClickHandle(marker: any, isSelectMarker: boolean = false) {
    marker.on('click', (event: any) => {
      ...
      if (this.selectMarker) {
        if (this.selectMarker.getExtData().parking.parking_id === info.parking.parking_id) {
          this.zIndex = 102;
        }
      }

      if (!isSelectMarker) {
        // 如果有搜索选中的marker在放大状态，缩小他
