...
export class EditBasicInfoComponent implements OnInit {
  ...
  public requestParkingByIdData() {
    ...
    this.parkingsHttpService.requestParkingsByIdData(this.parking_id).subscribe(data => {
      ...
      // 某个停车场没有parking_type字段
      // if (!isNullOrUndefined(this.parkingsInfo.parking_type)) {
      //   this.parkingsInfo.parking_type.forEach(groupType => {
      //     this.parkingsType.forEach(type => {
      //       if (type === groupType) {
      //         $('#parking_type_' + groupType).prop('checked', true);
      //       }
      //     });
      //   });
      // }
