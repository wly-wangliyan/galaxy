...
import {GroupEditEntity, GroupsHttpService} from '../groups-http.service';
...
import {CheckboxCheckedState} from "../../../../utils/common-enums";
...
@Component({
  ...
  styleUrls: ['../groups.component.css', './groups-add.component.css'],
  providers: [ParkingsHttpService]
})
export class GroupsAddComponent implements CanDeactivateComponent {

  public groupsInfo: GroupEditEntity = new GroupEditEntity();
  ...
  @ViewChild('createGroupsForm') public createGroupsForm: any;
  ...
  public onAddGroupsFormSubmit() {
    ...
    this.groupsInfo.parking_group_types = this.temSelectedGroupArr.join();
    this.groupsInfo.parking_ids = parking_ids.join();

    this.groupsHttpService.requestAddGroupsData(this.groupsInfo).subscribe(() => {
  ...
  public canDeactivate(): boolean {
    return this.editSuccess || !this.createGroupsForm || (!this.createGroupsForm.dirty && !this.groupTypeDirty && !this.parkingDirty);
  }
}
