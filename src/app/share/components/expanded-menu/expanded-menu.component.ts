...
export class ExpandedMenuComponent implements AfterViewInit {
  ...
  constructor(private router: Router, public authService: AuthService, private routeMonitorSerivce: RouteMonitorService) {
    ...
    this.menuItems.push(this.generateExceptionsMenu());
    // this.menuItems.push(this.generateBeianMenu());
    this.menuItems.push(this.generateBasicsMenu());
    this.menuItems.push(this.generateEmployeesMenu());
    this.menuItems.push(this.generateLogMenu());
  }
  ...
  private generateDataStatisticsMenu(): ExpandedMenuItem {
    ...
    const subMenuData3 = new ExpandedMenuItemData('终端统计');
    subMenuData3.paths[0] = '/data-statistics/terminal';
    subMenuData3.permissionSettings = ['data_statistics'];
    this.routeLinkList.push(subMenuData3);
    ...
    menuItem.lowerLevels = [subMenuItem1, subMenuItem2, subMenuItem3, subMenuItem4];
    this.firstLevelMenuItems.push(menuItem);
    this.secondLevelMenuItems.push(subMenuItem1);
    this.secondLevelMenuItems.push(subMenuItem2);
    this.secondLevelMenuItems.push(subMenuItem3);
  ...
  private generateDataRecordsMenu(): ExpandedMenuItem {
    ...
    // const subMenuData2 = new ExpandedMenuItemData('订单记录');
    // subMenuData2.paths[0] = '/records/order';
    // subMenuData2.permissionSettings = ['data_record'];
    // this.routeLinkList.push(subMenuData2);

    const subMenuData3 = new ExpandedMenuItemData('上传记录');
    subMenuData3.paths[0] = '/records/upload';
    subMenuData3.permissionSettings = ['data_record'];
    this.routeLinkList.push(subMenuData3);
    ...
    // const subMenuItem2 = new ExpandedMenuItem(subMenuData2, menuItem);
    const subMenuItem3 = new ExpandedMenuItem(subMenuData3, menuItem);
    menuItem.lowerLevels = [subMenuItem1, subMenuItem3];
    this.firstLevelMenuItems.push(menuItem);
    this.secondLevelMenuItems.push(subMenuItem1);
    // this.secondLevelMenuItems.push(subMenuItem2);
    this.secondLevelMenuItems.push(subMenuItem3);
  ...
  /* 异常警告 */
  private generateExceptionsMenu(): ExpandedMenuItem {
    const menuData = new ExpandedMenuItemData('异常警告');
    menuData.normalIcon = 'menu-icon4';
    menuData.permissionSettings = ['exception_warning'];
    menuData.paths[0] = '/exceptions';
    this.routeLinkList.push(menuData);
    const menuItem = new ExpandedMenuItem(menuData);
    this.firstLevelMenuItems.push(menuItem);
    return menuItem;
  ...
  /* 系统日志 */
  private generateLogMenu(): ExpandedMenuItem {
    const menuData = new ExpandedMenuItemData('系统日志');
    menuData.normalIcon = 'menu-icon8';
    menuData.permissionSettings = ['system_log'];
    menuData.paths[0] = '/log';
    this.routeLinkList.push(menuData);
    const menuItem = new ExpandedMenuItem(menuData);
    this.firstLevelMenuItems.push(menuItem);
    return menuItem;
  ...
  /* 备案审核 */
  private generateBeianMenu(): ExpandedMenuItem {
    const menuData = new ExpandedMenuItemData('备案审核');
    menuData.normalIcon = 'menu-icon8';
    menuData.permissionSettings = ['beian_review'];

    const subMenuData1 = new ExpandedMenuItemData('停车场');
    subMenuData1.paths[0] = '/beian/parkings';
    subMenuData1.permissionSettings = ['beian_review'];
    this.routeLinkList.push(subMenuData1);

    const subMenuData2 = new ExpandedMenuItemData('管理系统');
    subMenuData2.paths[0] = '/beian/systems';
    subMenuData2.permissionSettings = ['beian_review'];
    this.routeLinkList.push(subMenuData2);
  ...
  /* 重置菜单栏 */
  public reset() {
    if (!isNullOrUndefined(this.currentSelectedItem)) {
      let upper = this.currentSelectedItem;
      while (!isNullOrUndefined(upper.upperLevel)) {
  ...
  public select(menuItem: ExpandedMenuItem) {
    if (!isNullOrUndefined(this.currentSelectedItem)) {
      ...
      if (leftMenuItem.data.menuId === rightMenuItem.data.menuId) {
        ...
        while (!isNullOrUndefined(leftMenuItem.upperLevel)) {
  ...
  private findItem(menuId: string, menuItems: Array<ExpandedMenuItem>): ExpandedMenuItem {
    for (const menuItem of menuItems) {
      if (menuItem.data.menuId === menuId) {
        ...
        if (!isNullOrUndefined(menuItem.lowerLevels)) {
          const findItem = this.findItem(menuId, menuItem.lowerLevels);
          if (!isNullOrUndefined(findItem)) {
            return findItem;
          }
        }
      }
