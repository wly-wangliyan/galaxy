...
export class GlobalService {
  ...
  constructor(private authService: AuthService, private httpService: HttpService, private regionHttpService: RegionHttpService, private groupsHttpService: GroupsHttpService) {
  }
  ...
  public http502Page: Http502PageComponent;
  ...
  public get permissionGroups(): Observable<Array<UserPermissionGroupEntity>> {
    if (isUndefined(this._permissionGroups)) {
      return this.authService.requestPermissionGroups().map(permissionGroups => {
  ...
  public get groups(): Observable<Array<GroupEntity>> {
    if (isNullOrUndefined(this._groups)) {
      return this.groupsHttpService.requestAllGroupsData().map(groups => {
        this._groups = groups;
        return groups;
      });
    } else {
  ...
  public getRegionById(region_id: string): Observable<Array<RegionEntity>> {
    return this.regions.flatMap(regions => {
      // 处理
      const targetRegions = [];
      if (region_id) {
  ...
  public get regions(): Observable<Array<RegionEntity>> {
    if (isUndefined(this._regions)) {
      return this.regionHttpService.requestRegions().map(regions => {
  ...
  public get currentRegions(): Observable<Array<RegionEntity>> {
    if (isUndefined(this._currentRegions)) {
      return this.regionHttpService.requestRegions(GlobalConst.RegionID).map(regions => {
        const tempList = [];
        // 当前数据结构不复杂，可以这么干
  ...
  public get currentTileRegions(): Observable<Array<RegionEntity>> {
    return this.currentRegions.map(regions => {
      const tempList = [];
  ...
  public httpErrorProcess(err: any): boolean {
    ...
    if (err.status === 403) {
      ...
      if (!ScreenHelper.Instance.isFullScreen) {
        this.http502Page.http502Flag = true;
      }
      return true;
    } else {
      console.error(err);
