...
export class RoutePreventService implements CanDeactivate<CanDeactivateComponent> {
  ...
  public canDeactivate(component: CanDeactivateComponent, currentRoute: ActivatedRouteSnapshot, currentState: RouterStateSnapshot, nextState?: RouterStateSnapshot): boolean | Observable<boolean> {
    ...
    if ((this.globalService.http403Page && this.globalService.http403Page.http403Flag) || (this.globalService.http500Page && this.globalService.http500Page.http500Flag) || (this.globalService.http502Page && this.globalService.http502Page.http502Flag)) {
      // 当页面报错,500,502或403时也不阻止页面跳转
      return true;
    }
